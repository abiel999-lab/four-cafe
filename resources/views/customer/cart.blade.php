...
<div class="rounded-2xl bg-white/70 border border-black/10 p-4">
    ...
    @if(empty($cart))
        ...
        <a href="{{ route('customer.menu') }}" class="inline-block mt-3 px-4 py-2 rounded-xl bg-brand-primary text-brand-surface">Kembali ke Menu</a>
    @else
        <div class="flex flex-col gap-3">
            @foreach($cart as $item)
                @php
                    $line = ((int)$item['unit_price'] + (int)$item['options_total']) * (int)$item['quantity'];
                    $subtotal += $line;
                @endphp
                ...
                <div class="rounded-xl border border-black/10 bg-white/60 p-3">
                    <div class="flex justify-between gap-2">
                        <div>
                            ...
                            <div class="text-sm opacity-80">
                                {{ Money::rupiah((int)$item['unit_price'] + (int)$item['options_total']) }} x {{ $item['quantity'] }}
                            </div>
                    ...
                    <div class="mt-3 flex gap-2 items-center">
                        <form method="POST" action="{{ route('customer.cart.update') }}" class="flex gap-2 items-center">
                            ...
                            <button class="px-3 py-2 rounded-xl bg-brand-primary text-brand-surface">Update</button>
                        </form>

                        <form method="POST" action="{{ route('customer.cart.remove') }}">
                            @csrf
                            <input type="hidden" name="key" value="{{ $item['key'] }}">
                            <button class="px-3 py-2 rounded-xl border border-black/10 bg-white/60">Hapus</button>
                        </form>
                    </div>
                </div>
            @endforeach
        ...
        <div class="mt-4 flex gap-2">
            <a href="{{ route('customer.menu') }}" class="px-4 py-2 rounded-xl border border-black/10 bg-white/60">Tambah Menu</a>
            <a href="{{ route('customer.checkout.show') }}" class="flex-1 px-4 py-2 rounded-xl bg-brand-primary text-brand-surface text-center">
                Checkout
            </a>
        </div>
    @endif
