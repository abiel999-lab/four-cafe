  ...
  <div class="mx-auto max-w-5xl">
    <div class="rounded-2xl bg-white/60 border border-black/10 p-4">
      ...
      <div class="mt-3 flex flex-wrap gap-2">
        <a
          href="{{ route('customer.menu', ['category' => 'all', 'q' => $q]) }}"
        ...
        @foreach($categories as $cat)
          <a
            href="{{ route('customer.menu', ['category' => $cat->id, 'q' => $q]) }}"
...
<div class="mx-auto max-w-5xl">
  ...
  @if(session('success'))
    <div class="mb-3 rounded-xl bg-green-100 p-3 text-green-900">
      {{ session('success') }}
    </div>
  @endif

  @if(session('error'))
    <div class="mb-3 rounded-xl bg-red-100 p-3 text-red-900">
      {{ session('error') }}
    </div>
  @endif
  ...
  <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
    @forelse($products as $p)
      @php
        // DB kamu simpan: "products/xxxxx.jpg"
        // Karena kamu sudah expose /products via symlink, kita pakai url langsung:
        $img = !empty($p->image_path) ? url('/' . ltrim($p->image_path, '/')) : null;
      @endphp
      ...
      <div class="rounded-2xl bg-white/70 border border-black/10 overflow-hidden">
        <div class="aspect-[4/3] bg-black/5">
          @if($img)
            ...
            >
            <div class="w-full h-full hidden place-items-center text-5xl font-extrabold text-brand-primary/70">
